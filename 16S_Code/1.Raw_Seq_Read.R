...
##  This script processes 2109KMI-0001                   ##
##                                                       ##
##  Nudibranch bacteria microbiome                       ##
##                                                       ##
...
# Load metadata ####
bact_meta <- readxl::read_xlsx("MetaData.xlsx")

# remove extra cols
full_meta <- bact_meta %>% select(c("Sample ID","Species", "Island"))
...
rm(bact_meta); rm(full_meta) # quick cleanup of environment

# Peek at quality profiles
plotQualityProfile(fnFs[c(1,30)]) # fwd reads # drops below Q30 at around 220
plotQualityProfile(fnRs[c(1,30)]) # rev reads # drops below Q30 at around 150
...
FWD <- "TATGGTAATTGTGTGCCAGCMGCCGCGGTAA" # Sequence of FWD primer
REV <- "AGTCAGTCAGCCGGACTACHVGGGTWTCTAAT"  # Sequence of REV primer
...
filtFs <- list.files("./2109KMI-0001/filtered", pattern = "_F_filt", full.names = TRUE)
filtRs <- list.files("./2109KMI-0001/filtered", pattern = "_R_filt", full.names = TRUE)
...
ps@sam_data$Site <- ps@sam_data$Island
ps@sam_data$Site[8:34] <- "Pulau Hantu"
ps@sam_data$Site[55:56] <- "Pulau Hantu"
